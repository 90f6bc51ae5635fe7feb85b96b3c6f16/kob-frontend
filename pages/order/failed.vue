<template>
  <div class="pb-3">
      ...
      <b-col class="">
        <div class="row-header-content d-flex justify-content-between align-items-center">
          ...
          <nuxt-link :to="{ path: `/order/failed` }" class="header d-flex justify-content-center align-items-center"
            style="width: 100%; height: 50px">ไม่สำเร็จ</nuxt-link>
        </div>
      </b-col>
    </b-row>

    <div class="mt-4" v-for="(order, index) in orders" :key="index">
      <div v-if="order.order_status == 'payment_failed'" style="border: 1px solid #e5e5e5">
            ...
            <b-col cols="12" md="6">
              <div class="px-3 pb-3 pt-sm-3 bg-light-gray text-sm-right">
                <a v-b-modal.modal-2 @click="setOrderCode(order.order_code, order)">
                  <span style="color: #c82333" v-if="order.order_status == 'payment_failed'">
    ...
    <div v-if="failedordercount == 0" class="d-flex flex-column align-items-center justify-content-center"
      style="height: 600px">
